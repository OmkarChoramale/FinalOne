  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const [sitesRes, programsRes, eventsRes, usersRes, bookingsPagedRes, auditsRes] = await Promise.all([
            ...
            axios.get(`${BASE_URL}/audits/official`, axiosConfig).catch(() => ({ data: [] }))
        ]);
        ...
        // Count pending/planned audits from ComplianceService
        const pendingAudits = Array.isArray(auditsRes.data)
            ? auditsRes.data.filter(a => a.status === 'PLANNED' || a.status === 'IN_PROGRESS').length
            : 0;
